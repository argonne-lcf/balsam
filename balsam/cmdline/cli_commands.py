...
def rm(args):
    ...
    if objects_name.startswith("job"):
        cls = Job
    elif objects_name.startswith("app"):
        cls = AppDef
    objects = cls.objects
...
def server(args):
    ...
    if args.connect:
        ...
        uname = args.add_user
        if len(uname) == 0:
            raise RuntimeError("Please provide user name")
...
def make_dummies(args):
    ...
    if not App.objects.filter(name="dummy").exists():
        dummy_app = App(name="dummy", executable="echo hello, {{name}}!")
        dummy_app.save()
