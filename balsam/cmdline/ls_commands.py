...
try:
    LIMIT = int(environ.get("BALSAM_LS_LIMIT", 5000))
except ValueError:
    LIMIT = 5000
...
def print_jobs(jobs, verbose):
    ...
    if add:
        fields.extend([s.strip() for s in add.split(":") if s])
...
def print_subtree(job, indent=1):
    def job_str(job):
        return f"{job.name:10} {job.cute_id}"

    print("|" * indent, end=" ")
    print(5 * indent * " ", job_str(job))
    for job in job.children:
        print_subtree(job, indent + 1)


def print_jobs_tree(jobs):
    count = jobs.count()
    roots = jobs.filter(parent_ids=[])[:LIMIT]
    for job in roots:
        print_subtree(job)
    if count > LIMIT:
...
def ls_jobs(namestr, show_history, jobid, verbose, tree, wf, state, by_states):
    ...
    if namestr:
        results = results.filter(name__icontains=namestr)
    ...
    if wf:
        results = results.filter(workflow__icontains=wf)
    if state:
    ...
    if show_history:
        ...
        print_jobs_tree(results)
    elif by_states:
        print_by_states(results)
    else:
        print_jobs(results, verbose)
...
def ls_wf(name, verbose, tree, wf):
    ...
    if wf:
        name = wf  # wf argument overrides name
