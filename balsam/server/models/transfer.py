...
class TransferItemManager(models.Manager):
    ...
    def _create_stage_in(self, source, destination, job):
        try:
            ...
            raise ValidationError(
                f"Stage-in `source` URL is not formatted correctly: {e}"
            )

        destination = Path(destination)
        if destination.is_absolute():
            raise ValidationError(
                "Stage In `destination` must be a relative path to Job workdir"
            )
    ...
    def _create_stage_out(self, source, destination, job):
        try:
            ...
            raise ValidationError(
                f"Stage-out `destination` URL is not formatted correctly: {e}"
            )

        source = Path(source)
        if source.is_absolute():
            raise ValidationError(
                "Stage Out `source` must be a relative path to Job workdir"
            )
    ...
    def _parse_url(self, url):
        ...
        if not Path(path).is_absolute():
            raise ValueError(
                f"URL must end with an absolute path to a file or directory"
            )
...
class TransferItem(models.Model):
    ...
    protocol = models.CharField(
        max_length=16, editable=False, choices=TRANSFER_PROTOCOLS
    )
    state = models.CharField(
        max_length=32, default="pending", choices=TRANSFER_ITEM_STATES
    )
    direction = models.CharField(
        max_length=4, choices=[("in", "Stage In"), ("out", "Stage Out")]
    )
    ...
    job = models.ForeignKey(
        "Job", related_name="transfer_items", editable=False, on_delete=models.CASCADE
    )
