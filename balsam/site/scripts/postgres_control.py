...
def add_user(db_path, uname):
    ...
    subprocess.run(
        f'''psql -d balsam -h {host} -p {port} -c \
        "CREATE ROLE {uname} WITH LOGIN PASSWORD 'foo123'; \
        ...
        REVOKE INSERT ON core_applicationdefinition FROM {uname}; \
        REVOKE UPDATE ON core_applicationdefinition FROM {uname}; \
        REVOKE DELETE ON core_applicationdefinition FROM {uname}; " \
        ''',
        shell=True
    )
...
def create_db(serverInfo):
    ...
    retcode = subprocess.Popen(f'initdb -D {db_path} -A scram-sha-256 -U postgres', shell=True).wait()
