...
def select_range(num_threads, thread_idx):
    ...
    processable = processable.order_by(
        "-state"
    )  # put AWAITING_PARENTS last to avoid starvation
    logger.debug(f"There are {processable.count()} processable jobs")
    if num_threads == 1:
        qs = processable
    else:
        qs = processable.annotate(
            first_pk_char=Substr(Cast("pk", CharField(max_length=36)), 1, 1)
        )
...
def refresh_cache(job_cache, num_threads, thread_idx):
    ...
    logger.debug(
        f"TransitionThread{thread_idx} will try to acquire: {[str(id)[:8] for id in to_acquire]}"
    )
...
def release_jobs(job_cache):
    ...
    release_jobs = [
        j.pk
        for j in job_cache
        if (j.state not in PROCESSABLE_STATES) or (j.state == "AWAITING_PARENTS")
    ]
    if release_jobs:
...
def fast_forward(job_cache):
    ...
    for job in stagein_jobs:
        ...
        if not hasRemote and not (hasParents and hasInput):
            job.state = "STAGED_IN"
    ...
    retry_jobs = (
        j
        for j in job_cache
        if j.state == "RUN_TIMEOUT"
        and j.auto_timeout_retry
        and not j.post_timeout_handler
    )
    ...
    errfail_jobs = (
        j
        for j in job_cache
        if j.state == "RUN_ERROR" and not (j.post_error_handler and j.postprocess)
    )
    ...
    stageout_jobs = (
        j
        for j in job_cache
        if j.state == "POSTPROCESSED" and not (j.stage_out_url and j.stage_out_files)
    )
...
def postprocess(job, *, error_handling=False, timeout_handling=False):
    ...
    if timeout_handling and job.state == "RUN_TIMEOUT":
        message = (
            f"{job.cute_id} Timeout handling didn't change job state: marking FAILED"
        )
        raise BalsamTransitionError(message)
...
def handle_timeout(job):
    if job.post_timeout_handler:
        ...
        raise BalsamTransitionError(
            f"{job.cute_id} no timeout handling: marking FAILED"
        )
