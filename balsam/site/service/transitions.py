...
class ProcessingThreads:
    def __init__(self, num_threads):
        ...
        self.source_thread = threading.Thread(
            target=self.thread_producer, args=(processing_queue,)
        )
        self.updater_thread = threading.Thread(
            target=self.thread_status_updater, args=(status_queue,)
        )
        self.worker_threads = [
            threading.Thread(
                target=self.thread_task_processor, args=(processing_queue, status_queue)
            )
    ...
    def thread_producer(self, processing_queue):
        while not EXIT_FLAG:
            ...
            if to_acquire:
                ...
                processable = processable.exclude(pk__in=self.processing_pks)[
                    :to_acquire
                ]
...
def run_preprocess(job):
    if not job.preprocess:
        return ProcessingThreads.status_msg(
            job, state="PREPROCESSED", msg="Skipped preprocess"
        )
    ...
    if retcode != 0:
        tail = get_tail(out)
        return status_msg(
            job, state="FAILED", msg=f"Preprocess returned {retcode}:\n{tail}"
        )
    else:
