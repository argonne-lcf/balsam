...
class GridFTPHandler:
    def pre_stage_hook(self):
        ...
        if p.returncode != 0:  # valid proxy does not exist so create one
            command = (
                str(settings.GRIDFTP_PROXY_INIT)
                + " -verify -debug -bits 2048 -valid 96:00"
            )
            ...
            if p.returncode:
                ...
                raise Exception(
                    "Error in pre_stage_hook, return value = " + str(p.returncode)
                )
            else:
                logger.debug(
                    "gridftp initialization completed, stdout:\n"
                    + out
                    + "\n stderr:\n"
                    + err
                )
    ...
    def stage_in(self, source_url, destination_directory):
        ...
        command = str(settings.GRIDFTP_GLOBUS_URL_COPY) + " -dbg -nodcau -r %s %s" % (
            source_url,
            destination_directory,
        )
        logger.debug("command=" + command)
    ...
    def stage_out(self, source_directory, destination_url):
        ...
        if destination_url[-1] != "/":
            destination_url += "/"
        command = str(
            settings.GRIDFTP_GLOBUS_URL_COPY
        ) + " -dbg -nodcau -cd -r %s %s" % (source_directory, destination_url)
        logger.debug("command=" + command)
        try:
            p = subprocess.Popen(
                command.split(" "), stdout=subprocess.PIPE, stderr=subprocess.PIPE
