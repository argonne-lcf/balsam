...
Whereas the `RESTClient` provides a lower-level interface rooted in exchanging JSON data over HTTP, the `balsam.api` defines Django ORM-like `Models` and `Managers` to emulate the original Balsam API:
